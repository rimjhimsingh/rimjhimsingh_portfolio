...
// Dynamically import the syntax highlighter with SSR disabled
...
const useSyntaxHighlighterStyles = () => {
...
export default function RoleCycler() {
  ...
  const syntaxStyles = useSyntaxHighlighterStyles()
...
export default function RoleCycler() {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto">
      ...
      {showCode && (
        <div
          className={`rounded-lg overflow-hidden transition-all duration-500 ${showCode ? "max-h-[800px] opacity-100" : "max-h-0 opacity-0"}`}
        >
          <div
            className={`p-2 ${theme === "dark" ? "bg-[#1E1E1E]" : "bg-[#f5f5f5]"} text-sm flex justify-between items-center`}
          >
            <span className="font-mono">RoleCycler.tsx</span>
            <span className="text-xs opacity-70">React Component</span>
          </div>
          {syntaxStyles ? (
            <SyntaxHighlighter
              language="tsx"
              style={theme === "dark" ? syntaxStyles.dark : syntaxStyles.light}
              customStyle={{
                margin: 0,
            ...
            // Fallback while styles are loading
            <pre
              className={`p-4 ${theme === "dark" ? "bg-[#1E1E1E] text-gray-300" : "bg-[#f5f5f5] text-gray-800"} overflow-auto max-h-[500px]`}
            >
              {codeString}
            </pre>
          )}
